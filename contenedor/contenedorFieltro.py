# contenedor/contenedorFieltro.py
...
try:
    ...
    from contenedor import Contenedor
try:
    from modelos import Fieltro # Modelo específico
except ImportError:
    print("Error: No se pudo importar 'Fieltro' desde 'modelos'.")
    class Fieltro: pass
...
try:
    ...
    print(f"ERROR CRÍTICO [contenedorFieltro]: No se pueden importar funciones de almacen: {e}")
    def conectar_db(): return None
    def registrar_entrada_almacen(obj, pid): pass

# --- Clase ContenedorFieltro (Sin cambios en lógica interna) ---
class ContenedorFieltro(Contenedor, ABC):
    def __init__(self, fecha_pedido, fecha_llegada, proveedor, numero_factura, observaciones, gastos, valor_conversion, contenido_fieltro=None):
        super().__init__(fecha_pedido, fecha_llegada, proveedor, numero_factura, observaciones, gastos, valor_conversion)
        self.contenido = contenido_fieltro if contenido_fieltro is not None else []

    def agregar_rollo_fieltro(self, espesor, ancho, largo, n_bobinas, metro_lineal_usd, valor_conversion):
        rollo_fieltro = Fieltro(espesor=espesor, ancho=ancho, largo=largo,
                                n_bobinas=n_bobinas, metro_lineal_usd=metro_lineal_usd)
        try:
            if rollo_fieltro.metro_lineal_usd is not None and valor_conversion is not None:
                 rollo_fieltro.metro_lineal_euro = float(rollo_fieltro.metro_lineal_usd) * float(valor_conversion)
            else: rollo_fieltro.metro_lineal_euro = None
            if rollo_fieltro.metro_lineal_euro is not None and rollo_fieltro.largo is not None and rollo_fieltro.n_bobinas is not None:
                 rollo_fieltro.precio_total_euro = rollo_fieltro.metro_lineal_euro * float(rollo_fieltro.largo) * int(rollo_fieltro.n_bobinas)
            else: rollo_fieltro.precio_total_euro = None
        except (ValueError, TypeError) as e:
            print(f"Error calculando precios base para Fieltro: {e}. Datos: {rollo_fieltro.__dict__}")
            rollo_fieltro.metro_lineal_euro = None; rollo_fieltro.precio_total_euro = None
        self.contenido.append(rollo_fieltro)
        print(f"  + Rollo Fieltro añadido: {espesor}mm (Factura: {self.numero_factura})")
    ...
    def agregar_contenido(self, item_fieltro: Fieltro):
        if isinstance(item_fieltro, Fieltro): self.contenido.append(item_fieltro)
        else: print(f"Error: Se intentó añadir un objeto que no es Fieltro: {type(item_fieltro)}")

    def eliminar_contenido(self, indice):
        try: del self.contenido[indice]
        except IndexError: print(f"Error: Índice {indice} fuera de rango al intentar eliminar Fieltro.")

    def editar_contenido(self, indice, item_fieltro: Fieltro):
        try:
            if not isinstance(item_fieltro, Fieltro): raise TypeError("El item proporcionado no es de tipo Fieltro")
            self.contenido[indice] = item_fieltro
        except IndexError: print(f"Error: Índice {indice} fuera de rango al intentar editar Fieltro.")
        except TypeError as e: print(f"Error al editar contenido Fieltro: {e}")

# ==============================================================================
# FUNCIÓN GUARDAR/ACTUALIZAR CONTENEDOR FIELTRO (Refactorizada para DB)
# ==============================================================================
def guardar_o_actualizar_contenedores_fieltro(lista_contenedores: List[ContenedorFieltro]):
    """
    Guarda o actualiza una lista de objetos ContenedorFieltro en la base de datos.
    (Lógica idéntica a Goma/PVC, adaptada para Fieltro).
    """
    print(f"\n--- Iniciando guardado/actualización DB para {len(lista_contenedores)} Contenedor(es) Fieltro ---")
    contenedores_procesados = 0
    contenedores_fallidos = 0

    for contenedor_obj in lista_contenedores:
        if not isinstance(contenedor_obj, ContenedorFieltro):
             print(f"Error: Objeto no es ContenedorFieltro. Tipo: {type(contenedor_obj)}. Saltando...")
        ...
        print(f"\nProcesando Contenedor Fieltro - Factura: {num_factura}")
        ...
        try:
            ...
            print(f"ERROR procesando contenedor Fieltro - Factura: {num_factura}. Error: {e}")
            traceback.print_exc()
    ...
    print(f"\n--- Finalizado guardado/actualización DB Contenedores Fieltro ---")
    print(f"    Procesados con éxito: {contenedores_procesados}")
    print(f"    Fallidos: {contenedores_fallidos}")
    print("-" * 60)

# contenedor/contenedorFieltro.py
# ... (imports y clase ContenedorFieltro como estaban antes) ...
...
try:
    ...
    print(f"ERROR CRÍTICO [cargar_contenedores_fieltro]: No se puede importar conectar_db: {e}")
    def conectar_db(): return None
try:
    from modelos import Fieltro
except ImportError:
    print("Error: No se pudo importar 'Fieltro' desde 'modelos'.")
    class Fieltro: pass # Dummy
...
def cargar_contenedores_fieltro() -> List[ContenedorFieltro]:
    ...
    try:
        conn = conectar_db()
        if conn is None:
            print("Error: No se pudo conectar a la DB para cargar Contenedores Fieltro.")
            return []
        ...
        for pedido_row in pedidos_db:
            ...
            for item_stock_row in items_stock_db:
                ...
                try:
                    ...
                    fieltro_obj.metro_lineal_euro = None
                    contenido_reconst.append(fieltro_obj)
        ...
        print(f"Error inesperado cargando contenedores Fieltro: {e_gen}")
        traceback.print_exc()
    finally:
        if conn:
            conn.close()
            print("Conexión DB cerrada después de cargar contenedores Fieltro.")
